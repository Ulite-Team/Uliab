...
//! constants both sides check. A plugin crate generates its bindings from
//! [`WIT`] with `wit_bindgen::generate!`. Because both sides read the same
//! `include_str!`ed text, the interface cannot drift between them.
//!
//! This crate also re-exports [`UlbConfig`] — the derive macro that turns
//! a typed config struct into both real deserialization and a machine-
//! readable schema embedded in the plugin's compiled `.wasm` as a custom
//! section (ARCHITECTURE.md §3.8).
//!
...
pub use ulb_config_macros::UlbConfig;
pub use ulb_config_macros::embed_schema;
